...
import { User, Mail, Phone, Lock, Gift, ShieldAlert } from "lucide-react";
...
const Register = () => {
  ...
  useEffect(() => {
    ...
    if (refParam) {
      ...
      localStorage.setItem("perfectmoto_ref_code", refParam.toUpperCase());
    } else {
      const savedRef = localStorage.getItem("perfectmoto_ref_code");
      if (savedRef) setFormData((prev) => ({ ...prev, referralCode: savedRef }));
    }
  }, [searchParams]);
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (formData.phone.trim().length !== 10 || !/^\d+$/.test(formData.phone)) {
      setErrorMessage("Please enter a valid 10-digit Indian phone number.");
      setIsSubmitting(false);
      return;
    }
    try {
      await registerUser(formData.name, formData.email, formData.phone, formData.password, formData.referralCode);
      localStorage.removeItem("perfectmoto_ref_code");
  ...
  const handleGoogleSignup = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        setErrorMessage(err.response?.data?.message || err.message || "Google sign-up failed.");
      }
    },
    onError: () => setErrorMessage("Google Account Enrollment rejected.")
  });
      ...
      <div className="w-full max-w-[640px] z-10" style={{ perspective: "1600px" }}>
        ...
        >
          ...
          <div className="flex flex-col items-center mb-6" style={{ transform: "translateZ(30px)" }}>
            ...
            />
            <h2
              className="text-[32px] font-black uppercase italic leading-none text-center"
              style={{ color: "#1a1a1a", letterSpacing: "-0.02em", fontFamily: "'Arial Black', Impact, sans-serif" }}
            >
              JOIN THE{" "}
              <span style={{ color: "#cc8800", textShadow: "0 0 16px rgba(255,184,0,0.3)" }}>CREW</span>
            </h2>
            <p className="text-[10px] font-bold uppercase tracking-[0.22em] mt-2" style={{ color: "#888" }}>
              Create Profile & Gear Up
            </p>
          </div>
          ...
          <form onSubmit={handleFormSubmit} style={{ transform: "translateZ(20px)" }}>
            {/* 2-column grid layout */}
            <div className="grid grid-cols-1 gap-3 mb-3">
              {/* Full Name - full width */}
              <div className="relative">
                <User className="absolute left-4 top-1/2 -translate-y-1/2" size={15} style={iconColor} />
                <input type="text" name="name" required value={formData.name} onChange={handleInputChange}
                  style={inputStyle} onFocus={inputFocus} onBlur={inputBlur}
                  className="w-full h-[48px] pl-[42px] pr-4 rounded-[16px] text-sm font-medium placeholder-[#999] focus:outline-none transition-all"
                  placeholder="Full Name" />
              </div>

              {/* Email - full width */}
              <div className="relative">
                <Mail className="absolute left-4 top-1/2 -translate-y-1/2" size={15} style={iconColor} />
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div className="relative">
                <Phone className="absolute left-4 top-1/2 -translate-y-1/2" size={15} style={iconColor} />
              ...
              <div className="relative">
                <Lock className="absolute left-4 top-1/2 -translate-y-1/2" size={15} style={iconColor} />
            ...
            <div className="relative mb-4">
              <Gift className="absolute left-4 top-1/2 -translate-y-1/2" size={15} style={iconColor} />
              <input type="text" name="referralCode" value={formData.referralCode} onChange={handleInputChange}
                disabled={!!searchParams.get("ref")}
                style={inputStyle} onFocus={inputFocus} onBlur={inputBlur}
                className="w-full h-[48px] pl-[42px] pr-4 rounded-[16px] text-sm font-semibold uppercase placeholder-[#999] focus:outline-none transition-all tracking-wider disabled:opacity-50"
            ...
            <button type="submit" disabled={isSubmitting}
              style={{
                ...
                color: "#111111", fontFamily: "'Arial Black', Impact, sans-serif", letterSpacing: "0.12em"
              }}
              className="w-full h-[50px] rounded-[16px] text-sm font-black uppercase disabled:opacity-50 transform active:translate-y-0.5"
            >
          ...
          {/* Divider */}
          <div className="flex items-center gap-3 my-4" style={{ transform: "translateZ(10px)" }}>
          ...
          <button type="button" onClick={() => handleGoogleSignup()}
            style={{ background: "rgba(255,255,255,0.6)", border: "1px solid rgba(0,0,0,0.1)", color: "#444", transform: "translateZ(15px)", boxShadow: "inset 0 1px 0 rgba(255,255,255,0.9), 0 2px 6px rgba(0,0,0,0.08)" }}
            className="w-full h-[48px] font-bold rounded-[16px] flex items-center justify-center gap-3 text-sm"
          >
          ...
          <p className="text-center mt-4 text-xs font-medium" style={{ color: "#777", transform: "translateZ(10px)" }}>
            Already registered?{" "}
            <Link to="/login" className="font-bold" style={{ color: "#cc8800" }}>Login here</Link>
          </p>

          {/* Bottom bar */}
          <div style={{ height: 1, background: "linear-gradient(90deg, transparent, rgba(255,184,0,0.5), transparent)", marginTop: 20, transform: "translateZ(40px)" }} />
        </div>
      </div>
    </div>
  );
